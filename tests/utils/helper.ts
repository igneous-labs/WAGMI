...
export async function getOnChainTime(connection: web3.Connection): Promise<number> {
	const parsedClock = await connection.getParsedAccountInfo(web3.SYSVAR_CLOCK_PUBKEY);

	const parsedClockAccount = (parsedClock.value!.data as web3.ParsedAccountData).parsed as any;
...
export function deriveClaimStatus(index: BN, distributor: web3.PublicKey) {
	return web3.PublicKey.findProgramAddressSync(
		[Buffer.from("ClaimStatus"), new Uint8Array(index.toBuffer("le", 8)), distributor.toBytes()],
		MERKLE_DISTRIBUTOR_PROGRAM_ID
	);
}
...
export function deriveTransaction(smartWallet: web3.PublicKey, txNo: BN) {
	return web3.PublicKey.findProgramAddressSync(
		[Buffer.from("Transaction"), smartWallet.toBytes(), new Uint8Array(txNo.toBuffer("le", 8))],
		SMART_WALLET_PROGRAM_ID
	);
}
...
export function deriveProposal(governor: web3.PublicKey, proposalCount: BN) {
	return web3.PublicKey.findProgramAddressSync(
		[Buffer.from("Proposal"), governor.toBytes(), new Uint8Array(proposalCount.toBuffer("le", 8))],
		GOVERN_PROGRAM_ID
	);
}
...
export async function createGovernor(
	...
	const [governor, _gBump] = deriveGovern(baseKeypair.publicKey);
	const [locker, _lBump] = deriveLocker(baseKeypair.publicKey, voterProgram);
...
export async function createMetLocker(
	...
	const onchainTimestamp = await getOnChainTime(voterProgram.provider.connection);
...
export async function getOrCreateVote(proposal: web3.PublicKey, governProgram: Program<Govern>) {
	const [vote, _bump] = deriveVote(governProgram.provider.publicKey, proposal);

	const voteAccount = await governProgram.provider.connection.getAccountInfo(vote);
...
export async function invokeAndAssertError(
	...
	try {
		...
		if (isAnchorError) {
			...
			expect(anchorError.error.errorMessage.toLowerCase()).to.be.equal(message.toLowerCase());
		} else {
			const logs: string[] = error.logs;
			expect(logs.find((s) => s.toLowerCase().includes(message.toLowerCase()))).to.be.not.undefined;
		}
	}
