...
describe("Locked voter", () => {
	...
	async function createLFGProposal() {
		...
		await createOptionProposalMeta(proposal, optionDescriptions, governProgram);
	...
	before(async () => {
		...
		rewardMint = await createMint(provider.connection, keypair, keypair.publicKey, null, 9);
		...
		// create reward mint
		votingRewardMint = await createMint(provider.connection, keypair, keypair.publicKey, null, 9);

		// set voting reward
		const governProgram = createGovernProgram(wallet, GOVERN_PROGRAM_ID);

		const ixData = governProgram.coder.instruction.encode("set_voting_reward", {
			rewardPerProposal: rewardPerProposal,
		});

		const smartWalletProgram = createSmartWalletProgram(wallet, SMART_WALLET_PROGRAM_ID);

		const smartWalletState = await smartWalletProgram.account.smartWallet.fetch(smartWallet);

		const [transaction, txBump] = deriveTransaction(smartWallet, smartWalletState.numTransactions);
	...
	it("users initialize new escrow and lock token", async () => {
		for (const keypair of userKeypairs) {
			...
			const escrowATA = await getOrCreateATA(rewardMint, escrow, keypair, provider.connection);
	...
	it("protocol team activates proposal", async () => {
		...
		const ixData = voterProgram.coder.instruction.encode("activate_proposal", {});
	...
	it("users cast for proposal and claim reward", async () => {
		...
		while (true) {
			...
			if (proposalState.votingEndsAt.toNumber() > onchainTimestamp) {
				console.log(
					`${proposalState.votingEndsAt.toNumber() - onchainTimestamp} seconds until proposal pass`
				);
				await sleep(1000);
			} else {
				break;
		...
		const proposalState = await governProgram.account.proposal.fetch(proposal);
		expect(proposalState.totalClaimedReward.toNumber() + 5).to.be.greaterThan(
			rewardPerProposal.toNumber()
		); // precision loss
	});
});
